...
@torch.no_grad()
def train_with_features(train_loader, model, logger, config, features_path):
    """
    测试训练集特征的分类准确率
    Args:
    ...
    header = 'Train with pre-extracted features:'
    ...
    # 加载预提取的特征
    logger.info(f"Loading pre-extracted training features from {features_path}")
    ...
    # 检查特征数量是否匹配
    if len(pre_extracted_features) != len(train_loader.dataset):
        logger.warning(f"Number of pre-extracted training features ({len(pre_extracted_features)}) "
                      f"does not match dataset size ({len(train_loader.dataset)})")
    ...
    if 'filenames' in features_data and features_data['filenames'] is not None:
        ...
        # 创建数据集文件名到索引的映射
        dataset_filenames = [sample.get('file_id', f"sample_{i}") for i, sample in enumerate(train_loader.dataset)]
        dataset_to_feature_idx = [filename_to_idx.get(fname, -1) for fname in dataset_filenames]
    else:
        # 如果没有文件名，假设顺序匹配
        dataset_to_feature_idx = list(range(min(len(pre_extracted_features), len(train_loader.dataset))))

    logger.info(f"{config.num_clip * config.num_crop} views inference with pre-extracted training features")
    ...
    # 处理每个批次
    batch_idx = 0
    for batch_data in metric_logger.log_every(train_loader, config.print_freq, logger, header):
        ...
        for i in range(b):
            idx = dataset_to_feature_idx[batch_idx * train_loader.batch_size + i]
    ...
    logger.info(f' * Train Acc@1 {metric_logger.acc1.global_avg:.3f} Train Acc@5 {metric_logger.acc5.global_avg:.3f}')
    ...
    for i, (name, acc) in enumerate(zip(class_names, class_acc)):
        logger.info(f'Train {name}: {acc*100:.2f}%')
